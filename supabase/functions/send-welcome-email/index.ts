...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const { email, firstName, lastName, loginUrl }: WelcomeEmailRequest = await req.json();
    
    const appLoginUrl = loginUrl || LOGIN_URL;
    console.log(`Sending welcome email to: ${email} from: ${RESEND_FROM_EMAIL}, loginUrl: ${appLoginUrl}`);
    ...
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: [email],
        subject: "Welcome to Zira EduSuite!",
        html: `
          <!DOCTYPE html>
          <html>
            <head>
                ...
                <tr>
                  <td style="background-color: #ffffff; padding: 40px; border-radius: 0 0 16px 16px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.07);">
                    ...
                    <p style="color: #475569; font-size: 16px; line-height: 1.6; margin: 0 0 24px 0;">
                      Thank you for joining Zira EduSuite. We're excited to have you on board!
                    </p>
                    <p style="color: #475569; font-size: 16px; line-height: 1.6; margin: 0 0 24px 0;">
                      Zira EduSuite is a comprehensive education management platform designed to streamline operations for educational institutions across Africa.
                    </p>
                    ...
                    <p style="color: #64748b; font-size: 14px; text-align: center;">
                      Or visit: <a href="${appLoginUrl}" style="color: #0d9488;">${appLoginUrl}</a>
                    </p>
